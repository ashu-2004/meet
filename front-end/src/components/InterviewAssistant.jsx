...
const InterviewAssistant = ({ localStream }) => {
  ...
  const resetInterview = async () => {
    try {
      ...
      await fetch("http://localhost:3004/api/transcriptions/clear", {
        method: "POST",
      });

      // Reset on Python backend too (if needed)
      try {
        const response = await fetch("http://localhost:3004/api/reset", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const extractSections = (markdownText) => {
    ...
    Object.entries(sections).forEach(([key, value]) => {
      console.log(
        `Section [${key}] content (${value.length} chars):`,
  ...
  const analyzeResponse = async () => {
    ...
    try {
      ...
      if (followUpContent) {
        // Handle both bullet points or single line question
      ...
      alert(
        "Error connecting to the Python analysis backend. Make sure it's running on port 8000."
      );
    } finally {
